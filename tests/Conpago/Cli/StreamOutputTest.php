<?php
	...
	class StreamOutputTest extends \PHPUnit_Framework_TestCase {
		...
		function testWrite_willEchoNothing()
		{
			$o = fopen('php://memory', 'w');
			$out = new StreamOutput($o);
			$out->write();
		...
		function testWriteLine_willEchoData()
		{
			$o = fopen('php://memory', 'w');
			$out = new StreamOutput($o);
		...
		function testWriteLine_willEchoDataWithArgs()
		{
			$o = fopen('php://memory', 'w');
			$out = new StreamOutput($o);
		...
		function testWriteLine_willEchoEmptyLine()
		{
			$o = fopen('php://memory', 'w');
			$out = new StreamOutput($o);
			$out->writeLine();
